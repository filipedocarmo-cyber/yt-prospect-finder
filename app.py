# Correção do erro de comparação de data (TypeError)
# e melhoria na detecção de datas inválidas antes do filtro dos últimos 7 dias.
...
with st.sidebar:
    ...
    st.caption("A seção **Em Alta** usa critérios fixos: ≤20k inscritos, ≥20min, ≥10k views, publicados nos últimos 7 dias.")
...
def search_videos(service,query,region,published_after_iso,limit:int)->List[str]:
    ...
    while len(video_ids)<limit:
        res=service.search().list(
            part="id", type="video", order="viewCount", q=query, regionCode=region,
            publishedAfter=published_after_iso, maxResults=min(50,limit-len(video_ids)),
            pageToken=page_token, safeSearch="none"
        ).execute()
...
def get_videos_stats(service,video_ids:List[str])->pd.DataFrame:
    ...
    for ids_batch in chunked(video_ids,50):
        ...
        for it in res.get("items",[]):
            ...
            rows.append({
                "videoId":it.get("id"), "title":sn.get("title"), "publishedAt":sn.get("publishedAt"),
                "channelId":sn.get("channelId"), "channelTitle_video":sn.get("channelTitle"),
                "views":safe_int(stt.get("viewCount")), "likes":safe_int(stt.get("likeCount")),
                "comments":safe_int(stt.get("commentCount")), "duration_min":parse_duration_minutes(cd.get("duration","PT0M"))
            })
...
def get_channels_stats(service,channel_ids:List[str])->pd.DataFrame:
    ...
    for ids_batch in chunked(channel_ids,50):
        ...
        for it in res.get("items",[]):
            ...
            rows.append({"channelId":it.get("id"),"channelTitle_channel":sn.get("title"),"subs":safe_int(stt.get("subscriberCount"),-1)})
    return pd.DataFrame(rows)
...
if st.button("🚀 Buscar canais agora",type="primary"):
    ...
    ch_df=get_channels_stats(service,sorted(videos_df["channelId"].dropna().unique().tolist()))
    ...
    # ✅ Correção: Remover linhas sem data válida e converter timezone para UTC
    ...
    trending=merged[
        ...
        (merged["duration_min"]>=20)&(merged["views"]>=10_000)&
    ...
    if trending.empty:
        st.info("Nenhum vídeo em alta dentro desses critérios nesta busca.")
    else:
        st.dataframe(trending[["title","views","duration_min","publishedAt","channelTitle","subs","videoUrl","channelUrl"]],use_container_width=True)
else:
