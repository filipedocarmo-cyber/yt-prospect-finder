...
- 🔥 **Seção Em Alta (7 dias)**: canais ≤ 20k inscritos, vídeos ≥ 20min, ≥ 10k views, publicados nos últimos 7 dias
- 🟢 **Toggle** “Somente Em Alta” (oculta a tabela geral quando ligado)
- 🗂️ **Filtro de Categoria** (People & Blogs, Entertainment etc.) — usa as categorias oficiais do YouTube para a região escolhida
- ✅ Correções de data (`NaT`) e unificação de `channelTitle`
...
if st.button("🚀 Buscar canais agora", type="primary"):
    ...
    # 3) Seção Em Alta (critérios fixos)
    ...
    st.caption(
        "Canais ≤ 20k inscritos • Vídeos ≥ 20 min • ≥ 10k views • Publicados nos últimos 7 dias"
    )
    ...
    st.download_button(
        ...
        file_name=f"yt_trending_7d_{ts}.csv",
        mime="text/csv",
    ...
    if not show_only_trending:
        general = merged.copy()
        general = general[
            (general["views"] >= min_views_general)
            & (general["duration_min"] >= min_duration_general)
        ]
